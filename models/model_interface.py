...
class DTI_prediction(pl.LightningModule):
    ...
    def step(self, batch):
        mol_feature, prot_feat_student, prot_feat_teacher, text_features, property, y, source = batch        
        prot_feat_teacher = prot_feat_teacher.detach()
        output, lambda_ = self.model(mol_feature, prot_feat_student, prot_feat_teacher, text_features, property)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('valid_lambda_', lambda_, on_step=False, on_epoch=True, prog_bar=False)
        
        return {'preds': preds.float(), 'target': y}
